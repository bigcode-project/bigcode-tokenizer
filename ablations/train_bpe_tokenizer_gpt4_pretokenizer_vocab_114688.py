...
gpt4_regex = Regex(r"""(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\r\n\p{L}\p{N}]?\p{L}+|\p{N}{1,3}| ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+""")
gpt4_pretokenizer = pre_tokenizers.Split(pattern=gpt4_regex, behavior="removed", invert=True)
...
tokenizer.pre_tokenizer = pre_tokenizers.Sequence([gpt4_pretokenizer, bytelevel_pretokenizer])
...
tokenizer_wrapper.push_to_hub("bigcode/tokenizer-ablations", revision="gpt4-pretokenizer-vocab-114_688")